...
def check_type(file_path):
    ...
    """
    assert (Path(file_path).exists()) and (Path(file_path).is_file()
                                           ), "Your input file path is not valid or the file doesn't exist."
...
def extract_audio(file_path, format: str = "wav"):
    ...
    command = "ffmpeg -y -i "+file_path + \
        " -ab 160k -ac 2 -ar 44100 -vn \"" + mv_audio_file + "\""
