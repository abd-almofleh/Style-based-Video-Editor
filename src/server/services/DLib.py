...
class DLib:
  ...
  def detect_faces_and_landmarks(imagePath, cnn=False):
    ...
    DETECTOR_PATH = str(Path(__file__).parent.parent.joinpath("weights/mmod_human_face_detector.dat").resolve())

    Detector = dlib.get_frontal_face_detector() if not cnn else dlib.cnn_face_detection_model_v1(DETECTOR_PATH)
  ...
  def saveFaces(imagePath, boxes, name=""):
    ...
    for (x, y, w, h) in boxes:
      ...
      path = (root / f"{name[:endIndex]}-{counter}.jpg").absolute().resolve()
  ...
  def ExtractFaces(imagePath):
    ...
    for i in range(len(rectangles)):
      ...
      landmarksObj = dict()
      landmarksObj["left_eye"] = [(landmark[37].x + landmark[40].x)/2.0, (landmark[37].y + landmark[40].y)/2.0]
      ...
      landmarksObj["right_eye"] = [(landmark[43].x + landmark[46].x)/2.0, (landmark[43].y + landmark[46].y)/2.0]
